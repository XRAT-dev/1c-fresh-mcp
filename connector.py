...
НДС_СТАВКИ = {
    ...
    20:  "НДС20",    # была основной до 2025, для переходных документов
    10:  "НДС10",    # льготная (еда, детские товары, книги, медикаменты)
    7:   "НДС7",     # УСН с доходом 250-450 млн ₽ (с 2025)
    5:   "НДС5",     # УСН с доходом 60-250 млн ₽ (с 2025)
    0:   "НДС0",     # экспорт (с вычетом входного НДС)
    None:"БезНДС",   # освобождение ст. 149 НК, без вычета
}
# Дробные доли ставки для расчёта суммы НДС из суммы С НДС
НДС_ДОЛИ = {
    "НДС22": 0.22, "НДС20": 0.20, "НДС10": 0.10,
    "НДС7":  0.07, "НДС5":  0.05, "НДС0":  0.0, "БезНДС": 0.0,
}
# Значение по умолчанию читается из .env (FRESH_VAT_DEFAULT); fallback — НДС22
try:
    from config import VAT_DEFAULT as НДС_ПО_УМОЛЧАНИЮ
except Exception:
...
class Fresh1C:
    ...
    def make_item(nom_guid: str, qty: float, price: float,
                  description: str = "", nds: str = None,
                  line_num: int = 1) -> Dict:
        """
        Создать строку товара для счёта/реализации.
        nds: "НДС22"/"НДС20"/"НДС10"/"НДС7"/"НДС5"/"НДС0"/"БезНДС".
             Если None — берётся FRESH_VAT_DEFAULT из .env (по умолчанию НДС22).
        """
        if nds is None:
            nds = НДС_ПО_УМОЛЧАНИЮ
        сумма = qty * price
        # Вычислить сумму НДС из ставки (цена включает НДС)
        default_доля = НДС_ДОЛИ.get(НДС_ПО_УМОЛЧАНИЮ, 0.22)
        доля = НДС_ДОЛИ.get(nds, default_доля)
        сумма_ндс = round(сумма * доля / (1 + доля), 2) if доля else 0.0
        return {
